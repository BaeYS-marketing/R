...
#복수형으로 노드 추출
totalViewElements<-remDr$findElements(using='css','span.u_cbox_in_view_comment')
sapply(totalViewElements,function(x){x$clickElement()})

#단수형으로 노드 추출
totalViewElements<-remDr$findElement(using='css','span.u_cbox_in_view_comment')
totalViewElements$clickElement()
totalViewElements$getElementTagName()
