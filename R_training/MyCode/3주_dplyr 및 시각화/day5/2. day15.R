...
#1.데이터 셋이 특별해서 그런지 R구문은 아니라, 이 패키지에서 korpopmap에 접근할 떄 @이를 써서 접근해야 한다.
#2. 이 패키지는 euc로 만들어졌기 떄문에 UTF-8-로 다시 인코딩 해줘야한다. 
...
#addPolygons : 구별로 다각형으로 잘라주고 stroke _ 겉 선 smoothFacor 부드릅게  , fillopacitu = 투명도
map4<-leaflet(korpopmap2) %>% addTiles() %>% setView(lat=37.559957 ,lng=126.975302 , zoom=11)%>%                 #group 없어도 된다. 
...
#광진구 구분 데이타 ... 어떻게 찾아?? : 뭐 출력을 하든 눈으로 찾든...했겠지지
...
data = quakes[1:20,]
mycol <- colorNumeric("Set1",data$mag)
