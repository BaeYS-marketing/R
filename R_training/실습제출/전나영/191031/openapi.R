...
doc1 <- htmlParse(doc, encoding = "UTF-8")
text2 <- xpathSApply(doc1, "//item/title", xmlValue)
text2
...
top <- xmlRoot(doc)
top
df <- xmlToDataFrame(getNodeSet(doc, "//itemList"), stringsAsFactors = FALSE) # 구성 요소가 factor라서 character로 바꿔줘야 함
df
str(df)

cat("[ 360번 버스정보 ]\n",
    "노선ID : ", df$busRouteId[1], "\n",
    "노선길이 : ", df$length[1], "\n",
    "기점 : ", df$stStationNm[1], "\n",
    "종점 : ", df$edStationNm[1], "\n",
    "배차간격 : ", df$term[1])
