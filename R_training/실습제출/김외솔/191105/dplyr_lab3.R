...
midwest <- midwest %>% 
  mutate(ratioadultstototal=popadults/poptotal)
...
# 10-3
midwest %>% 
  mutate(grade=ifelse(ratioadultstototal>=0.4,"large",ifelse(ratioadultstototal>=0.3,"middle","small")))

# 10-4
midwest <- midwest %>% 
  mutate(ratioasiantototal=popasian/poptotal)
midwest %>% 
