...
colnames(GOO) = c('구별', 'name', '일인가구')
goo = GOO %>% filter(구별 == '강남구')

korpopmap3@polygons = korpopmap3@polygons[which(korpopmap3@data$name %in% goo$name)[-1]]
korpopmap3@data = korpopmap3@data[which(korpopmap3@data$name %in% goo$name)[-1],]


korpopmap3@data$name = gsub('·','',korpopmap3@data$name) 

korpopmap3@data = merge(korpopmap3@data, goo, by.x='name', sort=FALSE)
...
saveWidget(gangnam, "oneMap.html")