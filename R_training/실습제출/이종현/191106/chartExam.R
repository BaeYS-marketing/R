...
barplot(my_vec, col=terrain.colors(length(my_vec)), cex=0.8, cex.axis=0.8, names.arg = my_names, xlab = "상품ID", ylab = "클릭수", main="세로바 그래프 실습")
dev.off()
...
res2 = time %>% as.factor %>% table

png(filename="clicklog2.png", bg="white") # 출력을 png파일로 설정
pie(res2, labels=paste(names(res2), "~", as.character(as.numeric(names(res2))+1)), col=rainbow(10), main="파이그래프 실습")
dev.off()

pie(my_vec2, labels=paste(my_names2, "~", as.character(as.numeric(my_names2)+1)), col=rainbow(10), main="파이그래프 실습")
