#문제1
a <- array(1:24, dim=c(2,3,4))
a[2,3,4]
a[2,,]
a[,1,]
a[,,3]
a+100
a[,,4]*100
a[1,c(2,3),]
a[2,,2]<-a[2,,2]+100
a[,,1]<-a[,,1]-2
a<-a*10
rm(a)

#문제2
x<-c(1,2,3,4,5)
y<-c(2,4,6,8,10)
df1 <- data.frame(x,y)
df2 <- data.frame(col1=c(1,2,3,4,5), col2=c('a','b','c','d','e'), col3=c(6,7,8,9,10))

#문제4
제품명 <- c("사과", "딸기", "수박")
가격<- c(1800, 1500, 3000)
판매량 <-c(24,38,13)
df3<- data.frame(제품명, 가격, 판매량, stringsAsFactors=F)


#문제5
mean(df3$가격)
mean(df3$판매량)

#문제6
name<-c("Potter", "Elsa", "Gates", "Wendy", "Ben")
gender<-factor(c("M","F","M","F","M"))
math<-c(85,76,99,88,40)
df4<-data.frame(name,gender,math)
df4$name=as.character(df4$name)


str(df4)
stat<-c(76,73,95,82,35)
df4<- cbind(df4, stat)
df4$score <- df4$math + df4$stat
df4$grade <- ifelse(df4$score>=150, "A", 
                    ifelse(df4$score>=100 & df4$score<150, "B",
                           ifelse(df4$score>=70 & df4$score<100, "C", "D")))
emp<-read.csv("data/emp.csv")
emp
str(emp)
emp[c(3,4,5),]
subset(emp, select=c("ename"))
subset(emp, select=c("ename","sal"))
subset(emp, subset=(job=='SALESMAN'), select=c("ename", "sal", "job"))
#문제12
subset(emp, subset=(sal>=1000 & sal<=3000), select=c("ename","sal","deptno"))
#문제13
subset(emp, subset=(job!="ANALYST"), select=c("ename","job","sal"))
#문제14
subset(emp, subset=(job==c("ANALYST","SALESMAN")), select=c("ename","job"))
#문제15
subset(emp, subset=(is.na(emp$comm)), select=c("ename","sal"))
#문제16
emp[order(emp$sal),]
