...
# 문제13
emp %>% group_by(deptno) %>% summarise(n = n()) %>% arrange(desc(n)) %>% head(1)[1]


# 문제14
str(emp$ename)
enamechr <- as.character(emp$ename)

emp %>% mutate(enamelength = nchar(enamechr)) %>% arrange(enamelength)


# 문제15
emp %>% filter(commrate != "NA") %>% summarise(n=n())